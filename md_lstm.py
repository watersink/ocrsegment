...
def horizontal_standard_lstm(input_data, rnn_size, scope_n="layer1"):
    with tf.variable_scope("MultiDimensionalLSTMCell-" + scope_n):
        # input is (b, h, w, c)
        b, _, _, c = input_data.get_shape().as_list()
        h,w=tf.shape(input_data)[1],tf.shape(input_data)[2]
        # transpose = swap h and w.
        new_input_data = tf.reshape(input_data, (b * h, w, c))  # horizontal.
        rnn_out, _ = tf.nn.bidirectional_dynamic_rnn(
                tf.contrib.rnn.LSTMCell(rnn_size//2), 
                tf.contrib.rnn.LSTMCell(rnn_size//2), 
...
def snake_standard_lstm(input_data, rnn_size, scope_n="layer1"):
    with tf.variable_scope("MultiDimensionalLSTMCell-" + scope_n):
        # input is (b, h, w, c)
        b, _, _, c = input_data.get_shape().as_list()
        h,w=tf.shape(input_data)[1],tf.shape(input_data)[2]
...
def horizontal_vertical_lstm_inorder(input_data, rnn_size, scope_n="layer1"):
    with tf.variable_scope("MultiDimensionalLSTMCell-horizontal-" + scope_n):
        # input is (b, h, w, c)
        #horizontal
        b_h, _, _, c_h = input_data.get_shape().as_list()
        h_h,w_h=tf.shape(input_data)[1],tf.shape(input_data)[2]
        # transpose = swap h and w.
        ...
        rnn_out_h = tf.reshape(rnn_out_h, (b_h, h_h, w_h, rnn_size))
        #vertical
    with tf.variable_scope("MultiDimensionalLSTMCell-vertical-" + scope_n):
        new_input_data_v=tf.transpose(rnn_out_h,(0,2,1,3))
        b_v, _, _, c_v = new_input_data_v.get_shape().as_list()
        h_v,w_v=tf.shape(new_input_data_v)[1],tf.shape(new_input_data_v)[2]
        ...
        rnn_out_v = tf.reshape(rnn_out_v, (b_v, h_v, w_v, rnn_size))
...
def horizontal_vertical_lstm_together(input_data, rnn_size, scope_n="layer1"):
    with tf.variable_scope("MultiDimensionalLSTMCell-horizontal-" + scope_n):
        ...
        rnn_out_h, _ = tf.nn.bidirectional_dynamic_rnn(
                ...
                time_major=False)
        rnn_out_h=tf.concat(rnn_out_h, 2)
        rnn_out_h = tf.reshape(rnn_out_h, (b_h, h_h, w_h, rnn_size//2))
        #vertical
    with tf.variable_scope("MultiDimensionalLSTMCell-vertical-" + scope_n):
        ...
        rnn_out_v = tf.reshape(rnn_out_v, (b_v, h_v, w_v, rnn_size//2))
        rnn_out_v=tf.transpose(rnn_out_v,(0,2,1,3))
